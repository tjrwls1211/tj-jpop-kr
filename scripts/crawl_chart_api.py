...
def load_artist_aliases():
    """Load ja->ko mapping file (tsv with 'ja_name\\tko_name')."""
    mapping = {}
...
def crawl_tj_jpop_chart():
    """Call TJ chart API and return song list."""
    songs = []
...
def update_database(songs):
    ...
    for song in songs:
        cursor.execute("SELECT id FROM songs WHERE tj_number = ?", (song["tj_number"],))
    ...
    cursor.execute("SELECT COUNT(*) FROM songs WHERE is_confirmed = 0")
